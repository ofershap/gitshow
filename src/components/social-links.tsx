      ...
      <div className="flex flex-col gap-2">
        {links.map((link) => (
          <a
            ...
            className="group flex items-center gap-3 rounded-xl px-4 py-2.5 text-sm text-zinc-300 transition-all hover:text-white"
            style={{ background: "var(--color-surface)", border: "1px solid var(--color-border)" }}
          >
            <span className="text-zinc-400 group-hover:text-teal-400 transition-colors">{link.icon}</span>
            {link.label}
            <ArrowUpRightIcon />
          </a>
        ))}
...
function ArrowUpRightIcon() {
  return (
    <svg className="ml-auto h-3.5 w-3.5 text-zinc-600 transition-transform group-hover:translate-x-0.5 group-hover:-translate-y-0.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
